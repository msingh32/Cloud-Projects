...
aws ec2 run-instances --count $1 --image-id ami-d933e5a1 --key-name $2 --security-group-ids $3 --instance-type t2.micro --iam-instance-profile Name=$4 --user-data file://install-app-env.sh
...
aws autoscaling create-launch-configuration --launch-configuration-name $5 --image-id ami-d933e5a1 --key-name $2 --instance-type t2.micro --user-data file://install-app-env.sh --security-groups $3 --iam-instance-profile $4
